...
def main():
    ...
    df.to_csv(Path(args.output_dir, 'angles_csv.csv'))
...
def calc_crossing_incident_angle(model, mhc_type, Bchain = 'E', Achain = 'D', mhc_chain1 = 'M', mhc_chain2 = 'N'):
    pdb = pdb2sql(model)
    if mhc_type == "MH1":
        #xyzCYS = pdb.get('x,y,z', chainID = [Achain, Bchain], resSeq = [23, 104], name = ['CA'])
        xyzCYS_D = pdb.get('x,y,z', chainID = [Achain], name = ['CA'], resName = ['CYS'], resSeq = [22, 23, 104])
        xyzCYS_E = pdb.get('x,y,z', chainID = [Bchain], name = ['CA'], resName = ['CYS'], resSeq = [21, 23, 104])
        ...
        ref_pointTCR =pdb.get('x,y,z', chainID = [Achain], resSeq = ['110'], name = ['CA'])#97
