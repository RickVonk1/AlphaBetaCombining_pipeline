...
Every Python script within this pipeline follows a standard internal structure of distinct parts:
*   **Parts:** Explanation, Import, Inputs, Code, Activation.
*   **Explanation:** Quickly explain the exact function of the script and outline which input variables are modifiable.
*   **Import:** The import modules required for script function.
*   **Input:** Clearly isolated the variables that must be adjusted by the user before executing the script.
*   **Code:** The code used within the script for its function.
*   **Activation:** The activation of the functions written in the code section.
---
...
*   This is a shorted version of the pipeline made by D. Marzella for the RadboudUMC, as such it holds a different structure than the rest of the scripts.
...
*   **Script 2:** Filters out models displaying reversed docking orientations. It also generates a brief run-quality summary and compresses (`.tar`) the AlphaFold generated intermediary files.
...
*   **Analysis:** Extracts HADDOCK response and adds it to the results folder than archives the HADDOCK folder into a `.tar` file.
...
*   This script is meant to be executed when all chains have been processed.
